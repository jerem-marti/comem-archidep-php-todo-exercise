...
define('DB_PASS', getenv('TODOLIST_DB_PWD'));
...
if (isset($_POST['action'])) {
  switch($_POST['action']) {
    ...
    case 'toggle':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $updateQuery = $updateQuery = "UPDATE todo SET done = 1-done WHERE id = $id"; // add by Schäfer
    ...
    case 'delete':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $deleteQuery = "DELETE  from `todo` WHERE `id` = $id"; // Add by Maeva
 ...
 */
$selectQuery = 'SELECT * FROM `todo`ORDER BY done, created_at'; // Add by Jérémy
